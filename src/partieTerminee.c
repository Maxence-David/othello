...
int OTH_plateauPlein(PL_Plateau plateau)
{
    ...
    i = 1;
    while (!caseVide && i<= 8)  /*On itère sur les colonnes tant que la case n'est pas vide et qu'on ne sort pas du plateu*/
    {
        j = 1;
        while (!caseVide && j <= 8) /*On itère sur les lignes tant que la case n'est pas vide et qu'on ne sort pas du plateu*/
        {
...
int OTH_plusDeCoups(PL_Plateau plateau, CO_Couleur couleurJoueurCourant)
{
    ...
    i = 1;
    while (!coupOK && i <= 8)   /*On itère sur les colonnes tant que le coup n'est pas valide n'est pas vide et qu'on ne sort pas du plateu*/
    {
        j = 1;
        while (!coupOK && j <= 8)   /*On itère sur les lignes tant que le coup n'est pas valide et qu'on ne sort pas du plateu*/
        {
